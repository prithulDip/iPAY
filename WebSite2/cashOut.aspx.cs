...
public partial class cashOut : System.Web.UI.Page
{
    ...
    {
        if(AID.Text != "" && Amount.Text != "" &&  Password.Text!= "")
        {
            conn.Open();
            string com = "SELECT count(1) from Agents where phone=@phone";
            SqlCommand cmd = new SqlCommand(com, conn);
            cmd.Parameters.AddWithValue("@phone", AID.Text);
            int check = Convert.ToInt32(cmd.ExecuteScalar());
            if (check != 1)
            {
                Error.Text = "Agent does not exists";
                Error.Visible = true;
                return;
            }
            ...
            com = "SELECT MAX(T_id) as t_id FROM cashout";
            ...
            com = "INSERT into cashout (T_id,From_ID,Agent_ID,amount,date_time) VALUES(@ID,@from,@to,@amount,@time)";

            cmd = new SqlCommand(com, conn);
            cmd.Parameters.AddWithValue("@ID", newTID);
            cmd.Parameters.AddWithValue("@from", phone);
            cmd.Parameters.AddWithValue("@to", AID.Text);
